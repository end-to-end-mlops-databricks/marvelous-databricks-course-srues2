...
from sleep_efficiency.data_processor import ProjectConfig
import json
from mlflow import MlflowClient
from mlflow.utils.environment import _mlflow_conda_env
from sleep_efficiency.utils import adjust_predictions
...
run_id = mlflow.search_runs(
    experiment_names=["/Shared/sleep_efficiency"],
    filter_string="tags.branch='week1and2_stanruessink'",
).run_id[0]
...
# COMMAND ----------
wrapped_model = SleepEfficiencyModelWrapper(model) # we pass the loaded model to the wrapper
...
mlflow.set_experiment(experiment_name="/Shared/sleep-efficiency-pyfunc")
...
with mlflow.start_run(tags={"branch": "week1and2_stanruessink",
    ...
    mlflow.pyfunc.log_model(
        ...
        artifact_path="pyfunc-sleep-efficiency-model",
        code_paths = ["mlops_with_databricks-0.0.1-py3-none-any.whl"], #if you run in databricks UI, path can be Volume. Now I'm running locally
        signature=signature
    )

# COMMAND ----------
loaded_model = mlflow.pyfunc.load_model(f'runs:/{run_id}/pyfunc-sleep-efficiency-model')
loaded_model.unwrap_python_model()

# COMMAND ----------
model_name = f"{catalog_name}.{schema_name}.sleep_effciency_model_pyfunc"

model_version = mlflow.register_model(
    model_uri=f'runs:/{run_id}/pyfunc-sleep-efficiency-model',
    name=model_name,
